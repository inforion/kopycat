...
class ARMInstructionsTest: Module(null, "ARMv7InstructionTest") {
    ...
    private val memorySize = 0x20000L
    private val arm = ARMv7Core(this, "arm", 48.MHz, 1.0)
    private val ram = RAM(this, "ram", memorySize.asInt)
    ...
    private fun assemble(instruction: String): ByteArray {
        val prc = Runtime.getRuntime().exec(arrayOf("kstool", "arm", instruction))
        val error = prc.errorStream.bufferedReader().readLine()
        assertNull(error, "Decoding error!")
        val raw = prc.inputStream.bufferedReader().readLine().removePrefix(instruction)
    ...
    private fun flags(n: Long = 0, z: Long = 0,
        ...
        unicorn.reg_write(UC_ARM_REG_APSR, arm.cpu.sregs.apsr)
    }
    ...
    private fun status(q: Long = 0, ge: Long = 0) {
        ...
        unicorn.reg_write(UC_ARM_REG_CPSR, arm.cpu.sregs.cpsr)
    }
    ...
    private fun assertRegisters() {
        assertRegister(0,  unicorn.regRead(UC_ARM_REG_R0),  arm.cpu.regs.r0)
        assertRegister(1,  unicorn.regRead(UC_ARM_REG_R1),  arm.cpu.regs.r1)
        assertRegister(2,  unicorn.regRead(UC_ARM_REG_R2),  arm.cpu.regs.r2)
        assertRegister(3,  unicorn.regRead(UC_ARM_REG_R3),  arm.cpu.regs.r3)
        assertRegister(4,  unicorn.regRead(UC_ARM_REG_R4),  arm.cpu.regs.r4)
        assertRegister(5,  unicorn.regRead(UC_ARM_REG_R5),  arm.cpu.regs.r5)
        assertRegister(6,  unicorn.regRead(UC_ARM_REG_R6),  arm.cpu.regs.r6)
        assertRegister(7,  unicorn.regRead(UC_ARM_REG_R7),  arm.cpu.regs.r7)
        assertRegister(8,  unicorn.regRead(UC_ARM_REG_R8),  arm.cpu.regs.r8)
        assertRegister(9,  unicorn.regRead(UC_ARM_REG_R9),  arm.cpu.regs.r9)
        assertRegister(10, unicorn.regRead(UC_ARM_REG_R10), arm.cpu.regs.r10)
        assertRegister(11, unicorn.regRead(UC_ARM_REG_R11), arm.cpu.regs.r11)
        assertRegister(12, unicorn.regRead(UC_ARM_REG_R12), arm.cpu.regs.r12)
        assertRegister(13, unicorn.regRead(UC_ARM_REG_R13), arm.cpu.regs.spMain)
        assertRegister(14, unicorn.regRead(UC_ARM_REG_R14), arm.cpu.regs.lr)
    }
    ...
    private fun assertMemory() {
        Assert.assertArrayEquals("${arm.cpu.insn} -> Memory error",
                unicorn.mem_read(startAddress, memorySize),
                arm.load(startAddress, memorySize.asInt))
    }
    ...
    @Before fun resetTest() {
        arm.reset()
        unicorn.mem_map(startAddress, memorySize, UC_PROT_ALL)
    }
    ...
    @After fun checkPC() {
        ...
        unicorn.emu_stop()
        unicorn.mem_unmap(startAddress, memorySize)
    }
