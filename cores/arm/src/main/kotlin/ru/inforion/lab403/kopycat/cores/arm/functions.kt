...
//    assert(shift in 1..63)
    val result = LSL(x, N, shift)
    val carry_out = result[N].asInt
    return Pair(result, carry_out)
}

inline fun LSL(x: Long, N: Int, shift: Int): Long {
//    assert(shift in 0..63)
    return (x shl shift) mask N
}

inline fun LSR_C(x: Long, N: Int, shift: Int): Pair<Long, Int> {
//    assert(shift in 1..63)
    val result = LSR(x, N, shift)
    val carry_out = x[shift - 1].asInt
    return Pair(result, carry_out)
}

inline fun LSR(x: Long, N: Int, shift: Int): Long {
//    assert(shift in 0..63)
    return (x ushr shift) mask N
}
