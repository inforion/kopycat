...
class CharPointer constructor(val sys: System, address: Long = 0) : Pointer<Char>(address) {
    override fun get(index: Int) = sys.fullABI.readChar(address + index * sys.sizeOf.char).asChar
    override fun set(index: Int, value: Char) = sys.fullABI.writeChar(address + index * sys.sizeOf.char, value.asULong)
