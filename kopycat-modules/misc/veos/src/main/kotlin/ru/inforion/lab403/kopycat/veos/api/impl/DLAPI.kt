...
class DLAPI(os: VEOS<*>) : API(os) {
        ...
        override fun exec(name: String, vararg argv: ULong): APIResult {
            ...
            if (filenameString[0] == '/') {
                if (!os.filesystem.exists(filenameString))
                    return retval(0uL) // TODO: error processing
    ...
    val dlsym = object : APIFunction("dlsym") {
        ...
        override fun exec(name: String, vararg argv: ULong): APIResult {
            ...
            val result = os.loader.findSymbol(module, symbolString)?.address ?: 0uL
            check (result != 0uL) { "Symbol $symbolString not found in $module" }
            return retval(result)
        }
    }
