...
class Definition<T>(initialize: Definition<T>.() -> Unit) {
    ...
    private lateinit var successAction: (T) -> ULong

    private lateinit var failureAction: (error: Exception) -> ULong

    fun execute(action: () -> T) = run { executeAction = action }

    fun success(action: (T) -> ULong) = run { successAction = action }

    fun failure(action: (Exception) -> ULong) = run { failureAction = action }
