...
/**
 * [Sources on bootlin](https://elixir.bootlin.com/linux/v3.1/source/arch/x86/include/asm/thread_info.h#L26)
 *
 * @see LinuxThreadInfo
 */
class Linux0301ThreadInfo(offsetable: IOffsetable) : LinuxThreadInfo {
    ...
    inner class Raw {
        ...
        val execDomainPtr = OffsetField(_offsetable, "exec_domain", 0x8uL, Datatype.QWORD)
        val flags = OffsetField(_offsetable, "flags", 0x10uL, Datatype.DWORD)
        val status = OffsetField(_offsetable, "status", 0x14uL, Datatype.DWORD)
        val cpu = OffsetField(_offsetable, "cpu", 0x18uL, Datatype.DWORD)
        val preemptCount = OffsetField(_offsetable, "preempt_count", 0x1CuL, Datatype.DWORD)
        val addrLimit = OffsetField(_offsetable, "addr_limit", 0x20uL, Datatype.QWORD)
    }
