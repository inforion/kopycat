...
class FormatI(core: MSP430Core, val construct: constructor) : MSP430Decoder(core) {
    ...
    override fun decode(s: ULong): AMSP430Instruction {
        val aSrc = s[5..4].int
        val aDst = s[7].int
        val dtype = if (s[6] == 1uL) BYTE else WORD
        ...
        val op1 = decodeFirstOp(aSrc, src, word1, dtype, size)

        val imm1PC = MSP430Immediate(dtype, word1 - size + WORD.bytes, true)
        val imm2PC = MSP430Immediate(dtype, word2 - size + WORD.bytes, true)

        val op2: MSP430Operand = when (aDst) {
            0 -> MSP430Register.gpr(dtype, dest)
            1 -> when (dest) {
                0 -> MSP430Displacement(
                    dtype,
                    MSP430Register.gpr(WORD, dest),
                    if (fstImm) imm2PC else imm1PC,
                    WRITE,
                    0
                )
