...
import ru.inforion.lab403.kopycat.cores.arm.SRType.SRType_RRX
...
class RRX(cpu: AARMCore,
    ...
    override val mnem = "RRX${if(setFlags) "S" else ""}$mcnd"
    private var result = ARMVariable(Datatype.DWORD)

    override fun execute() {
        val (res, carry) = Shift_C(rm.value(core), 32, SRType_RRX, 1, core.cpu.flags.c.asInt)
        result.value(core, res)
        if (rd.isProgramCounter(core)) core.cpu.ALUWritePC(result.value(core))
        else {
            rd.value(core, result)
            if (setFlags) FlagProcessor.processLogicFlag(core, result, carry == 1)
        }
    }
}