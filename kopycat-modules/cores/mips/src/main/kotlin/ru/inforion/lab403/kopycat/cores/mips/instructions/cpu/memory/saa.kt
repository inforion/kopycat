...
) : FtOffsetInsn(core, data, Type.VOID, rt, base) {
    private val saad = base.dtyp == Datatype.QWORD
    override val mnem = "saa" + if (saad) "d" else ""

    override fun execute() {
        if (saad && !core.is64bit) {
            throw MipsHardwareException.RI(core.pc)
        }

        if (!saad && address[1..0].truth || saad && address[2..0].truth) {
