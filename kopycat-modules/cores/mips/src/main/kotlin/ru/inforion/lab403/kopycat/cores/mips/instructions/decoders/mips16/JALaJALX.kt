...
class JALaJALX(core: MipsCore,
         val constructLow: (MipsCore, ULong, MipsImmediate) -> AMipsInstruction) : ADecoder(core) {

    override fun decode(data: ULong): AMipsInstruction {
        require (!isExtended(data)) { "Not implemented" }

        val target = (data[4..0] shl 21) or (data[9..5] shl 16) or data[31..16]

        return constructLow(core, data, imm(target))
    }
}