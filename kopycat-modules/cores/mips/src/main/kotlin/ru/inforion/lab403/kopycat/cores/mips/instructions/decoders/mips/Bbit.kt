...
class Bbit(
    core: MipsCore,
    val construct: (MipsCore, ULong, Boolean, Boolean, MipsRegister, MipsImmediate, MipsNear) -> AMipsInstruction,
) : ADecoder(core) {
    override fun decode(data: ULong): AMipsInstruction {
        val rs = data[25..21].int
        val p = data[20..16]
        val offset = (data[15..0] shl 2).signext(17)
        val set = data[29].truth
        val add32 = data[28].truth

        return construct(core, data, set, add32, gpr(rs), imm(p), near(offset.int))
    }
}
