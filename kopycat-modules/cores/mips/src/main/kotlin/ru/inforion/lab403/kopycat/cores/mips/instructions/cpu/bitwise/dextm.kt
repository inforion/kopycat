...
/**
 *
 * dextm rt, rs, lsb, size
 *
 * To extract a bit field (63..32) from GPR rs and store it right-justified into GPR rt.
 */
...
class dextm(
    ...
    override val mnem = "dextm"

    override fun execute() {
        if (core.ArchitectureRevision == 1) throw MipsHardwareException.RI(core.pc)
        vrt = if (pos.value.int + siz.value.int <= 64) vrs[(lsb + msb + 32)..lsb] else 0u
    }
