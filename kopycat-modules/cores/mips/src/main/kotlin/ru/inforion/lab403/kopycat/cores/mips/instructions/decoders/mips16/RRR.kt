...
class RRR(core: MipsCore,
          val constructLow: (MipsCore, ULong, MipsRegister, MipsRegister, MipsRegister) -> AMipsInstruction) : ADecoder(core) {

    override fun decode(data: ULong): AMipsInstruction {
        require (!isExtended(data)) { "Not implemented" }

        val rx = data[10..8].int
        val ry = data[7..5].int
        val rz = data[4..2].int

        return constructLow(core, data, gpr16(rx), gpr16(ry), gpr16(rz))
    }
}