...
class Fxch(core: x86Core, opcode: ByteArray, prefs: Prefixes, vararg operands: AOperand<x86Core>) :
    AFPUInstruction(core, opcode, prefs, *operands) {
    override val mnem = "fxch"

    override fun executeFPUInstruction() {
        val temp = op1.extValue(core)
        op1.extValue(core, op2.extValue(core))
        op2.extValue(core, temp)
    }
}
