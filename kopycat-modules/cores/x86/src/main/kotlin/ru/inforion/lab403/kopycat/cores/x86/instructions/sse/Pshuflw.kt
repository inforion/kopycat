...
class Pshuflw(core: x86Core, opcode: ByteArray, prefs: Prefixes, vararg operands: AOperand<x86Core>) :
    ASSEInstruction(core, opcode, prefs, *operands) {

    override val mnem = "pshuflw"
    ...
    override fun executeSSEInstruction() {
        ...
        (0 until 4).forEach {
            ...
            val lsbOrder = it * 2
            val msbOrder = (it + 1) * 2 - 1
            val rangeOrder = msbOrder..lsbOrder

            val shiftSize = order[rangeOrder].int * 16
            result = result.insert(src ushr shiftSize, range)
        }

        result = result.insert(src[127..64], 127..64)
