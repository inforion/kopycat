...
class JmpDC(core: x86Core) : ADecoder<AX86Instruction>(core) {
    ...
    override fun decode(s: x86OperandStream, prefs: Prefixes): AX86Instruction {
        ...
        if (core.is64bit) when (opcode) {
            0xEB, 0xE9 -> prefs.rexW = true
        }
        return when (opcode) {
            0xEB -> Jmp(core, s.near8(prefs), s.data, prefs, isRelative = true, isFar = false)
            0xE9 -> Jmp(core, s.near(prefs), s.data, prefs, isRelative = true, isFar = false)
            0xEA -> Jmp(core, s.far(prefs), s.data, prefs, isRelative = false, isFar = true)
            0xFF -> {
                val sopcode = s.peekByte().int
                val row = sopcode[5..3]
                val rm = RMDC(s, prefs)
                when (row) {
                    0x04 -> {
                        if (core.is64bit) prefs.rexW = true
                        Jmp(core, rm.mpref, s.data, prefs, isRelative = false, isFar = false)
                    }
                    0x05 -> {
                        val mpref = rm.mpref
