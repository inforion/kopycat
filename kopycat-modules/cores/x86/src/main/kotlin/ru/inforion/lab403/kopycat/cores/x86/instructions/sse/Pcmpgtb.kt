...
class Pcmpgtb(core: x86Core, opcode: ByteArray, prefs: Prefixes, vararg operands: AOperand<x86Core>) :
    ASSEInstruction(core, opcode, prefs, *operands) {

    override val mnem = "pcmpgtb"

    override fun executeSSEInstruction() {
        val (dest, src) = when (op1.dtyp) {
        ...
        (0 until op2.dtyp.bytes).forEach {
            ...
            if (dest[range].byte > src[range].byte) {
                result = result set range
            }
        }
