...
class Btc(core: x86Core, opcode: ByteArray, prefs: Prefixes, vararg operands: AOperand<x86Core>):
    AX86Instruction(core, Type.VOID, opcode, prefs, *operands) {
    override val mnem = "btc"

    override fun execute() {
        val (a1, bitpos) = x86utils.bitBase(core, op1, op2, prefs)
        val cf = a1.bit(core, bitpos)
        core.cpu.flags.cf = cf.truth
        a1.bit(core, bitpos, (!cf.truth).int)
    }
}
