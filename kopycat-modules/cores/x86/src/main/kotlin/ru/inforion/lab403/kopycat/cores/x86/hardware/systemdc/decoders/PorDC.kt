...
import ru.inforion.lab403.kopycat.cores.x86.instructions.sse.Por
import ru.inforion.lab403.kopycat.cores.x86.instructions.sse.Pxor
import ru.inforion.lab403.kopycat.modules.cores.x86Core


class PorDC(core: x86Core) : ADecoder<AX86Instruction>(core) {
    override fun decode(s: x86OperandStream, prefs: Prefixes): AX86Instruction {
        val opcode = s.readOpcode()
        val rm = RMDC(s, prefs)

        val operands = when (opcode) {
            0xEB -> if (prefs.operandOverride) arrayOf(rm.rxmm, rm.xmmpref) else arrayOf(rm.rmmx, rm.mmxpref)
            else -> throw GeneralException("Incorrect opcode in decoder $this")
        }

        return Por(core, s.data, prefs, *operands)
    }
}