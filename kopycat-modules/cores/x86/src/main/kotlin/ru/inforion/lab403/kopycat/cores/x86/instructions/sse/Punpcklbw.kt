...
class Punpcklbw(core: x86Core, opcode: ByteArray, prefs: Prefixes, vararg operands: AOperand<x86Core>) :
    AX86Instruction(core, Type.VOID, opcode, prefs, *operands) {

    override val mnem = "punpcklbw"

    override fun execute() {
        val dest = op1.extValue(core)
        val src = op2.extValue(core)

        // INTERLEAVE_BYTES (DEST, SRC)
        var result = BigInteger.ZERO
        (0 until (op2.dtyp.bytes / 2)).forEach {
            // DST and SRC byte ranges
            val lsbFrom = it*8
            val msbFrom = (it + 1)*8 - 1
