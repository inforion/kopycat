...
//Shift left word
class slwx(core: PPCCore, val fieldA: Int, val fieldB: Int, val fieldC: Int, val flag: Boolean):
        APPCInstruction(core, Type.VOID) {
    override val mnem = "slw${if (flag) "." else ""}"
    ...
    override fun execute() {
        val n = rb.value(core)[4..0].int

        result.value(core, rs.value(core) shl n)

        ra.value(core, result)

        if (flag) FlagProcessor.processCR0(core, result)
    }
}