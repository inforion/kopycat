 ...
 *
 */
package ru.inforion.lab403.kopycat.cores.ppc.hardware.flags
...
object FlagProcessor {
    ...
    inline fun processCarryAlgShift(core: PPCCore, data: Variable<PPCCore>, n: Int) {
        core.cpu.xerBits.CA = data.isCarry(core) && (data.value(core) mask n != 0uL) //Won't work on 64 bit system
    }

    inline fun processOverflow(core: PPCCore, result: Variable<PPCCore>) {
        //result.isOverflow() - no works
        core.cpu.xerBits.OV = result.bit(core, result.msb(core)).truth xor result.isCarry(core)
