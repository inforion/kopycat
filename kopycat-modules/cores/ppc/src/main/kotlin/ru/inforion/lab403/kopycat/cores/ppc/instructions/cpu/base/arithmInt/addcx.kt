...
//Add carrying
class addcx(core: PPCCore, val overflow: Boolean, val record: Boolean, vararg operands: AOperand<PPCCore>):
        APPCInstruction(core, Type.VOID, *operands) {
    override val mnem = "addc${if (overflow) "o" else ""}${if (record) "." else ""}"
    override fun toString(): String = "$mnem $op1, $op2, $op3"

    private val result = PPCVariable(Datatype.DWORD)

    override fun execute() {
        result.value(core, op2.value(core) + op3.value(core))
