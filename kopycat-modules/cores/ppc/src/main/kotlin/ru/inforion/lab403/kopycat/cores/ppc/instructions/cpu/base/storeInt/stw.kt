...
//Store word
class stw(core: PPCCore, val condRegField: ULong, val length: Boolean, val data: ULong, vararg operands: AOperand<PPCCore>):
        APPCInstruction(core, Type.VOID, *operands) {
    override val mnem = "stw"

    override fun toString() = "$mnem $op1, ${data.signextRenameMeAfter(15).hex8}(${if ((op2 as PPCRegister).reg == eUISA.GPR0.id) "0" else op2.toString()})"
