...
class FormatIXCR(core: v850ESCore, val construct: constructor, val store: Boolean) : ADecoder<AV850ESInstruction>(core) {
    override fun decode(s: ULong) = if (store) {
        val regId = v850esRegister.creg(s[4..0].int)
        val reg2 = v850esRegister.gpr(s[15..11].int)
        construct(core, 4, arrayOf(regId, reg2))
    } else {
        val regId = v850esRegister.creg(s[15..11].int)
        val reg2 = v850esRegister.gpr(s[4..0].int)
