...
class TIMx(parent: Module, name: String, index: Int) : Module(parent, name) {
    ...
    private fun irqRequest() = ports.irq.request(0)
    private fun drqRequest() = ports.drq.request(0)
    ...
    ) : Register(ports.mem, register.offset, if (useDWORD) Datatype.DWORD else Datatype.WORD, register.name, default, writable, readable, level)
    ...
    private fun counterTriggerEvent() { // every timer tick
        TIMx_CNT.trigger()
        compareInterrupt()
    }

    private fun updateEvent(interrupt: Boolean = true) {  // if timer overload
        if (TIMx_CR1.OPM.toBool()) disableTimer()

        TIMx_PSC.updateShadow() // prescaler updates every time

        shadowedRegistersUpdate()
        
        if (interrupt) updateInterrupt()
    }

    private fun shadowedRegistersUpdate() {
        if (TIMx_CR1.ARPE.toBool() && !TIMx_CR1.UDIS.toBool() ) {
            TIMx_ARR.updateShadow()
        }

        if (TIMx_CCMR1.OC1PE.toBool()) TIMx_CCR1.updateShadow()
        if (TIMx_CCMR1.OC2PE.toBool()) TIMx_CCR2.updateShadow()
        if (TIMx_CCMR2.OC3PE.toBool()) TIMx_CCR3.updateShadow()
        if (TIMx_CCMR2.OC4PE.toBool()) TIMx_CCR4.updateShadow()
    }

    private fun interrupt(isIrq: Int, isDrq: Int) {
        if (isIrq.toBool()) irqRequest()
        if (isDrq.toBool()) drqRequest()
    }

    private fun updateInterrupt() {
        TIMx_SR.UIF = 1
        interrupt(TIMx_DIER.UIE, TIMx_DIER.UDE)
    }

    private fun triggerInterrupt() {
        TIMx_SR.TIF = 1
        interrupt(TIMx_DIER.TIE, TIMx_DIER.TDE)
    }

    private fun compareInterrupt(value: Int = TIMx_CNT.CNT) {
        if (TIMx_CCER.CC1E.toBool() && value == TIMx_CCR1.CCR) interruptCCR1()
        if (TIMx_CCER.CC2E.toBool() && value == TIMx_CCR2.CCR) interruptCCR2()
        if (TIMx_CCER.CC3E.toBool() && value == TIMx_CCR3.CCR) interruptCCR3()
        if (TIMx_CCER.CC4E.toBool() && value == TIMx_CCR4.CCR) interruptCCR4()
    }

    private fun interruptCCR1() {
        TIMx_SR.CC1IF = 1
        interrupt(TIMx_DIER.CC1IE, TIMx_DIER.CC1DE)
    }
    private fun interruptCCR2() {
        TIMx_SR.CC2IF = 1
        interrupt(TIMx_DIER.CC2IE, TIMx_DIER.CC2DE)
    }
    private fun interruptCCR3() {
        TIMx_SR.CC3IF = 1
        interrupt(TIMx_DIER.CC3IE, TIMx_DIER.CC3DE)
    }
    private fun interruptCCR4() {
        TIMx_SR.CC4IF = 1
        interrupt(TIMx_DIER.CC4IE, TIMx_DIER.CC4DE)
    }
    ...
    private val TIMx_EGR    = object : RegisterBase(RegisterType.TIMx_EGR, readable = false) {
        ...
        override fun write(ea: Long, ss: Int, size: Int, value: Long) {
            ...
            if (UG.toBool()) updateEvent(interrupt = !TIMx_CR1.URS.toBool())
            if (CC1G.toBool()) interruptCCR1()
            if (CC2G.toBool()) interruptCCR1()
            if (CC3G.toBool()) interruptCCR1()
            if (CC4G.toBool()) interruptCCR1()
            if (TG.toBool()) triggerInterrupt()
        }
    }
