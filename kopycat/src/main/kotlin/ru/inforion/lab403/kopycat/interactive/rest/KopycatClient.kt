...
class KopycatClient(host: String, port: Int, retries: Int = 10) {
    ...
    fun memLoad(address: ULong, size: Int, ss: Int): String =
            client.post("memLoad", mapOf("address" to address, "size" to size, "ss" to ss))

    fun memStore(address: ULong, data: String, ss: Int): Unit =
            client.post("memStore", mapOf("address" to address, "data" to data, "ss" to ss))

    fun regRead(index: Int) = client.post<ULong>("regRead", mapOf("index" to index))

    fun regWrite(index: Int, value: ULong) =
            client.post<Unit>("regWrite", mapOf("index" to index, "value" to value))

    fun pcRead() = client.post<ULong>("pcRead")
    fun pcWrite(value: ULong) = client.post<Unit>("pcWrite", mapOf("value" to value))
