...
import ru.inforion.lab403.kopycat.cores.base.getNumberLE
import ru.inforion.lab403.kopycat.cores.base.putNumberLE
 ...
 * {EN}
 */
class CachedMemoryStreamByteArray(
    ...
    private fun writeCache(value: ULong, size: Int) {
        cache.putNumberLE(loadedBytes, value, size)
        loadedBytes += size
    }

    private fun peekCache(size: Int) = cache.getNumberLE(readBytes, size)
    ...
    override fun read(datatype: Datatype): ULong {
        ...
        throw IndexOutOfBoundsException(
            "Cache size is $cacheSize. " +
                "Attempt to read byte at ${readBytes + datatype.bytes}"
        )
    }
