...
class KopycatRestProtocolIntegrationalTest {
    ...
    fun kopycatDefaultScenarioTest() = withKopycatRest(port) { kopycat, modules ->
        ...
        ram.endian = ByteOrder.BIG_ENDIAN

        val busResponse = client.bus(defaultModuleName, "mem")
        assertEquals(busResponse, "mem")
        assertEquals("$defaultModuleName:mem[Bus]", top.buses["mem"].toString())
        ...
        assertEquals(regResponse, 0x0uL)

        val pcResponse = client.pcRead()
        assertEquals(pcResponse, 0x1234uL)

        client.pcWrite(0x1000u)
        assertEquals(kopycat.core.pc, 0x1000uL)
        ...
        assertEquals(pc2Response, 0x1010uL)

        val reg2Response = client.regRead(2)
        assertEquals(reg2Response, 0uL)
    }
}
